...
scp -i $rsa_key ./docker-compose-webhook-production-override.yml $remote_server:$remote_folder/
scp -i $rsa_key ./envvars.txt $remote_server:$remote_folder/
scp -i $rsa_key ./docker-compose-webhook-startup-production.sh $remote_server:$remote_folder/
scp -i $rsa_key ./docker-all-images.sh $remote_server:$remote_folder/

# execute scripts on server
ssh -i $rsa_key $remote_server $remote_folder/docker-compose-webhook-startup-production.sh
